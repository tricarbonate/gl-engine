...
void Mesh::addAllPoints(btConvexHullShape* collisionShape) {
    for(size_t i = 0; i < vertices_.size(); i++) {
        btVector3 point = btVector3();
        point.setX(btScalar(vertices_[i].position.x));
        point.setY(btScalar(vertices_[i].position.y));
        point.setZ(btScalar(vertices_[i].position.z));
        collisionShape->addPoint(point);
    }
}
